...
const ManageExam = () => {
  ...
  const handleDelete = async (exam_title) => {
    try {
      await fetch(`http://localhost:4000/api/questions/objective/${exam_title}`, { method: 'DELETE' });
      await fetch(`http://localhost:4000/api/questions/subjective/${exam_title}`, { method: 'DELETE' });
